...
bool Game::init() {
    srand(static_cast<unsigned>(time(0))); // Inicializa la funci�n rand()
...
void Game::run() {
    while (running) {
        handleEvents();
        update();
        checkCollisions(); // Agrega esta l�nea
        render();
        SDL_Delay(16);
    }
}
...
void GreenCircle::init(SDL_Renderer* renderer) {
    ...
    if (surface) {
        ...
        printf("Error al cargar la imagen del c�rculo verde: %s\n", IMG_GetError());
    }
}
...
void Circle::init(SDL_Renderer* renderer) {
    ...
    if (surface) {
        ...
        printf("Error al cargar la imagen del c�rculo: %s\n", IMG_GetError());
    }
}
...
void Powerup::update() {
    // Aqu� puedes agregar la l�gica de actualizaci�n del power-up
}
...
void Game::handleEvents() {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        if (event.type == SDL_KEYDOWN) {
            ...
            case SDLK_SPACE: // Cambia SDLK_SPACE al bot�n que prefieras para agregar enemigos
...
void Game::drawMap() {
    // Colores
    SDL_Color darkBlue = { 20, 50, 100, 255 }; // Un azul oscuro m�s apagado
    SDL_Color lightBlue = { 40, 80, 130, 255 }; // Un azul medio m�s apagado
